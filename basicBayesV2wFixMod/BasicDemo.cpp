...
void BasicDemo::initPhysics(){
  ...
  const btVector3 bravoBoxHalfExtents( height_, height_, height_ );
  ...
  const btVector3 tangoBoxHalfExtents( 0.5f*height_, 0.5f*height_, 0.5f*height_ );
  ...
  btScalar tangoMass = 1.0f; //used to be 0.1f
  ...
  pGains_.setValue(50.0f,50.0f,50.0f);
  ...
  groundRigidBody_->setFriction(0.5);
  bravoRigidBody_->setFriction(0.5);
  tangoRigidBody_->setFriction(0.5);
...
std::vector<double> BasicDemo::runSimulation(std::vector<double> prevState, std::vector<double> action){
  height_ = 0.5f;
  startPose_.setValue(prevState[0],height_,prevState[1]);
  initPhysics();
  desiredPose_.setValue(action[0],height_,action[1]);
