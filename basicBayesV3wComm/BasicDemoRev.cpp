...
void BasicDemoRev::initPhysics(){
  ...
  */
  ...
  const btVector3 bravoBoxHalfExtents( beamLen_/2.0f, 0.5f, 0.5f );
  ...
  bravoTransform.setRotation(bravoQuat_);
  ...
  const btVector3 tangoBoxHalfExtents( 0.05f, 0.5f, 0.05f );
  ...
  const btVector3 pivotInB( -(beamLen_/2.0f), 0.0f, 0.0f );
  ...
  const btVector3 pivotInA2( (beamLen_/2.0f + 0.1f), 0.0f, 0.0f  );   
...
std::vector<double> BasicDemoRev::runSimulation(std::vector<double> prevState, std::vector<double> action){
  ...
  //in the case of the revolute joint, just pretend it starts aligned with the x axis if the angle is 0 and has a pivot such that the handle (tangoRigidBody) would be at the origin.

  //set parameters that are necessary and can't e hardcoded
  beamLen_ = 0.44-.1; //used to be 8.0
  beamAngle_ = prevState[0]; //M_PI/2.0;

  //std::cout << "beam start angle: " << beamAngle_ << std::endl;

  //get the revolute joint to start with the handle at 0 instead of the pivot at zero
  double handleX = 0.0;
  double handleY = 0.0;

  //***********************FIGURE THIS OUT*****************//
  //Calculate the required origins based on startPose_ given by the runSimulation function
  startPoseAlpha_.setValue(1.5+beamLen_+0.1f+handleX,5.0,0.0);
  startPoseTango_.setValue((startPoseAlpha_.getX()-1.5)+(beamLen_+0.1f)*cos(beamAngle_), 5.0f, (beamLen_+0.1f)*sin(beamAngle_));
  startPoseBravo_.setValue((startPoseAlpha_.getX()-1.5)+(beamLen_/2.0f)*cos(beamAngle_), 5.0f, (beamLen_/2.0f)*sin(beamAngle_));
  //startPoseAlpha_.setValue(-1.5,5.0,0.0);

  bravoQuat_.setValue(0.0f, sin(0.5f*-beamAngle_), 0.0f, cos(0.5f*-beamAngle_));
  //***********************END FIGURE THIS OUT*****************//
  ...
  if (debug_print_){
    ...
    std::cout << "start pose alpha: " << std::endl;
    std::cout << startPoseAlpha_[0] << std::endl;
    std::cout << startPoseAlpha_[1] << std::endl;
    std::cout << startPoseAlpha_[2] << std::endl;
    
    std::cout << "start pose bravo: " << std::endl;
    std::cout << startPoseBravo_[0] << std::endl;
    std::cout << startPoseBravo_[1] << std::endl;
    std::cout << startPoseBravo_[2] << std::endl;
  ...
  //this should definitely be the tango body
